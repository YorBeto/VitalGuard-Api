...
export class AdminService {
    ...
    private async getVitalIdUser(vitalId: string) {
        console.log(`\n=============================================================`);
        console.log(`🔍 [SSO-FETCH] Buscando identidad para el vital_id: ${vitalId}`);

        try {
            const vitalIdBaseUrl = process.env.VITAL_ID_API_URL;
            if (!vitalIdBaseUrl) {
                console.log(`⚠️  [SSO-FETCH] ERROR CRÍTICO: La variable VITAL_ID_API_URL no existe en tu .env`);
            }

            const endpoint = `${vitalIdBaseUrl}/auth/user/${vitalId}`;
            console.log(`🌐 [SSO-FETCH] Petición HTTP GET a: ${endpoint}`);

            const response = await fetch(endpoint);
            console.log(`📡 [SSO-FETCH] Código de respuesta: ${response.status} ${response.statusText}`);

            if (response.ok) {
                const responseJson = await response.json();

                // 👇 Entramos un nivel más profundo porque tu SSO lo envuelve en 'data'
                const userData = responseJson.data ? responseJson.data : responseJson;

                console.log(`📦 [SSO-FETCH] Datos extraídos:`, userData);
                ...
                // Si el SSO responde con 404, 500, etc.
                const errorText = await response.text();
                console.log(`❌ [SSO-FETCH] El SSO rechazó la petición. Detalle: ${errorText}`);
            }
        } catch (error: any) {
            console.error(`💥 [SSO-FETCH] Fallo a nivel de red (¿Está apagado el servidor 4000?):`, error.message);
        }

        console.log(`⚠️  [SSO-FETCH] Retornando datos de respaldo al frontend...`);
        console.log(`=============================================================\n`);

        // Si llegamos aquí, es porque algo falló arriba
    ...
    async generateGlobalPdfReport(): Promise<Buffer> {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                // Usamos la ruta exacta que usaste anteriormente
                doc.image('C:/Users/avina/OneDrive/Escritorio/VitalGuard-Api/logo.png', logoX, 40, { width: logoWidth });
            } catch (error) {
                console.log("Aviso: No se pudo cargar el logo de VitalGuard.");
            }
