...
export class DoctorsService {
  ...
  async generatePdfReport(
    ...
    return new Promise((resolve, reject) => {
      ...
      // LOGO CENTRADO
      try {
        const logoWidth = 220; // Tamaño del logo
        const logoX = (doc.page.width - logoWidth) / 2; // Cálculo para centrar perfectamente
        doc.image('C:/Users/avina/OneDrive/Escritorio/VitalGuard-Api/logo.png', logoX, 40, { width: logoWidth });
      } catch (error) {
        console.log("Aviso: No se pudo cargar el logo.");
      }
